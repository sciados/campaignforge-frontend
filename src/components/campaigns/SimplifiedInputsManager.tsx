...
const INPUT_TYPES: InputType[] = [
  ...
  },
  {
    ...
    placeholder:
      "Enter analytics data, conversion rates, performance metrics...",
    suggestedFor: ["business", "agency"],
    priority: 6,
  },
];
...
export default function SimplifiedInputsManager({
  ...
  const getRelevantInputTypes = useCallback(() => {
    ...
    return INPUT_TYPES.filter((inputType) =>
      inputType.suggestedFor.includes(userType)
  ...
  useEffect(() => {
    ...
    const initialInputs: CampaignInput[] = relevantInputTypes.map(
      (inputType) => ({
    ...
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-lg font-semibold text-gray-900">
          Campaign Input Sources
        </h3>
        <p className="text-sm text-gray-600 mt-1">
          Fill in any sources you have to power AI analysis and content
          generation
          {userType && (
        ...
        <p className="text-xs text-gray-500 mt-1">
          You do not need to fill all fields - just add what you have available
        </p>
      </div>
      ...
      <div className="border-t border-gray-200 pt-6">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onAnalyze}
            disabled={!hasValidInputs || hasInvalidInputs || isAnalyzing}
            className={`px-6 py-2 rounded-lg font-medium transition-colors ${
              hasValidInputs && !hasInvalidInputs && !isAnalyzing
