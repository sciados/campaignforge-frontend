...
const CAMPAIGN_TYPES = {
  universal: { label: 'Universal Campaign', icon: '🌟', color: 'bg-gray-100 text-black' }
}
...
function AppleCampaignCard({ 
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-lg transition-all hover:border-gray-300 group">
      ...
      {/* Content */}
      <h3 className="text-lg font-medium text-black mb-2 line-clamp-1">{campaign.title}</h3>
      <p className="text-apple-gray text-sm mb-4 line-clamp-2 leading-relaxed">{campaign.description}</p>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center justify-between text-sm">
          <span className="text-apple-gray font-medium">Intelligence Sources</span>
          <span className="font-semibold text-black">{campaign.intelligence_count ?? 0}</span>
        </div>
        <div className="flex items-center justify-between text-sm">
        ...
        {campaign.confidence_score && campaign.confidence_score > 0 && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-apple-gray font-medium">Confidence Score</span>
            <div className="flex items-center">
              <Star className="w-4 h-4 text-yellow-500 mr-1" />
              <span className="font-semibold text-black">{(campaign.confidence_score * 100).toFixed(0)}%</span>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="space-y-6">
                  ...
                  {SORT_OPTIONS.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-gray-100 transition-colors ${
                        sortBy === option.value ? 'bg-gray-100 text-black font-medium' : 'text-apple-gray'
                      }`}
                    >
                      <div className="flex items-center justify-between">
      ...
      {safeCampaigns.length === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-medium text-black mb-2">No campaigns found</h3>
          <p className="text-apple-gray">Try adjusting your search or filter criteria.</p>
        </div>
      ) : viewMode === 'grid' ? (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-apple-gray text-sm mb-2 line-clamp-1">{campaign.description}</p>
