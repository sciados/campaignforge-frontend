...
export default function AppleCampaignStats({
  campaigns,
  user,
}: CampaignStatsProps) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {stats.map((stat, index) => {
        ...
        return (
          <div
            key={index}
            className="bg-white rounded-2xl shadow-sm p-6 border border-gray-200 hover:shadow-lg transition-all"
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-apple-gray mb-1">
                  {stat.title}
                </p>
                <p className="text-3xl font-light text-black">{stat.value}</p>
              </div>
              <div
                className={`${stat.iconBg} w-12 h-12 rounded-xl flex items-center justify-center`}
              >
