// src/components/intelligence/ContentGenerator.tsx - UPDATED: Campaign Angles → Articles
import React, {
  useState,
  useCallback,
  useEffect,
...
import {
  ...
  Calendar,
  Edit3,
  BookOpen, // 📚 NEW: Added BookOpen icon for Articles
} from "lucide-react";
...
interface ContentTypeConfig {
  ...
  color: string;
  category: string;
  isComingSoon?: boolean; // 🆕 NEW: Flag for coming soon features
}

// 📝 UPDATED: Replaced Campaign Angles with Articles (Coming Soon)
const TEXT_CONTENT_TYPES: ContentTypeConfig[] = [
  ...
  {
    id: "articles", // 🔄 CHANGED: from 'campaign_angles' to 'articles'
    name: "Articles", // 🔄 CHANGED: from 'Campaign Angles' to 'Articles'
    icon: BookOpen, // 🔄 CHANGED: from '🎪' to BookOpen component
    description: "SEO-optimized articles and blog content", // 🔄 CHANGED: from '5 strategic marketing approaches'
    credits: 4, // 🔄 CHANGED: from 2 to 4 credits (articles are more complex)
    color: "bg-green-100", // 🔄 CHANGED: from 'bg-purple-100' to 'bg-green-100'
    category: "text",
    isComingSoon: true, // 🆕 NEW: Mark as coming soon
  },
];
...
const CONTENT_TYPES: ContentTypeConfig[] = [
  ...TEXT_CONTENT_TYPES,
  ...MEDIA_CONTENT_TYPES,
  ...COMPOSITE_CONTENT_TYPES,
];

const renderIcon = (
  icon: ContentTypeIcon,
  className: string = "h-6 w-6 text-gray-700"
) => {
...
export default function ContentGenerator({
  ...
  const loadContentItems = useCallback(async () => {
    if (isLoadingContent || !campaignId) {
      console.log(
        "⏸️ Skipping loadContentItems - already loading or no campaignId"
      );
      return;
    }
    ...
    try {
      ...
      if (
        apiResponse &&
        apiResponse.content_items &&
        Array.isArray(apiResponse.content_items)
      ) {
      ...
      const transformedItems: ContentItem[] = contentArray.map((item: any) => ({
        ...
        title:
          item.content_title ||
          item.title ||
          `${formatContentType(item.content_type)} Content`,
        created_at: item.created_at,
        confidence_score: item.confidence_score,
        is_published: item.is_published,
        user_rating: item.user_rating,
        preview_text:
          item.content_metadata?.preview || getPreviewText(item.content_body),
      }));

      setContentItems(transformedItems);
      console.log(
        "✅ Successfully set content items:",
  ...
  useEffect(() => {
    if (intelligenceSources.length === 1 && !selectedIntelligence) {
      ...
      console.log(
        "🎯 Auto-selecting single intelligence source:",
        singleSource.id
      );
      ...
      const sourceExists = intelligenceSources.some(
        (source) => source.id === selectedIntelligence
      );
      if (!sourceExists) {
        console.log(
          "🧹 Clearing invalid intelligence selection:",
  ...
  // 🆕 NEW: Handle coming soon content types
  const handleContentTypeClick = (contentType: ContentTypeConfig) => {
    if (contentType.isComingSoon) {
      alert(
        `📚 ${contentType.name} - Coming Soon!\n\nWe're working on bringing you AI-powered article generation. This feature will include:\n\n• SEO-optimized blog posts\n• Industry-specific articles\n• Content series planning\n• Keyword optimization\n\nStay tuned for updates!`
      );
      return;
    }
  ...
  const handleContentItemClick = async (contentItem: ContentItem) => {
    try {
      const fullContent = await stableApiMethods.getContentDetail(
        campaignId,
        contentItem.id
      );
  ...
  const handleContentSave = async (contentId: string, newContent: string) => {
    try {
      await stableApiMethods.updateContent(campaignId, contentId, {
  ...
  const formatContentType = (type: string): string => {
    const typeMap: Record<string, string> = {
      ...
      articles: "Articles", // 🆕 NEW: Added Articles mapping
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center shadow-sm">
        ...
        <p className="text-gray-600">
          Analyze salespage content first to generate intelligence-driven
          marketing materials.
        </p>
      </div>
    );
    ...
    <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setPreferences({ ...preferences, length: e.target.value })
                  }
                  className="w-full px-4 py-3 bg-gray-100 border-none rounded-lg focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all text-sm"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setPreferences({ ...preferences, platform: e.target.value })
                  }
                  className="w-full px-4 py-3 bg-gray-100 border-none rounded-lg focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all text-sm"
                >
                  <option value="general">General</option>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-black truncate">
                  {source.source_title || "Intelligence Source"}
                </h4>
                <div className="flex items-center">
              ...
              <p className="text-sm text-gray-600 capitalize">
                {(source.source_type || "unknown").replace("_", " ")}
              </p>
              {selectedIntelligence === source.id && (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-600">
            Select from 10 AI-powered content generators, organized by type
          </p>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Core text-based marketing content with psychological targeting
              </p>
            </div>
          </div>
            ...
            {TEXT_CONTENT_TYPES.map((contentType) => {
              return (
                <button
                  ...
                  className={`p-6 border-2 rounded-2xl text-left transition-all relative ${
                    selectedContentType === contentType.id
                      ? "border-blue-300 bg-blue-50"
                      : contentType.isComingSoon
        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Visual content generation with AI-powered creation
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="mb-6">
          <div className="flex items-center mb-4">
            ...
            <div>
              <h4 className="font-semibold text-gray-900">
                Composite Content Tools
              </h4>
              <p className="text-sm text-gray-600">
                Comprehensive multi-format content with strategic optimization
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="mt-8 bg-gray-50 rounded-xl p-6">
          <h4 className="font-semibold text-gray-900 mb-4">
            Content Generation System - Phase 2.3 Complete
          </h4>
      ...
      {generatedContent && (
        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
              ...
              <button
                onClick={() =>
                  downloadContent(
                    ...
                    `${generatedContent.content_type}.json`
                  )
                }
                className="flex items-center px-4 py-3 bg-gray-100 text-black rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium"
              >
          ...
          {generatedContent.performance_predictions && (
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {Object.entries(generatedContent.performance_predictions).map(
                  ([key, value]) => {
                    if (
          ...
          {generatedContent.smart_url && (
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={() =>
                    copyToClipboard(generatedContent.smart_url || "")
                  }
          ...
          {/* Usage Tips */}
          {generatedContent.generated_content.usage_tips &&
            generatedContent.generated_content.usage_tips.length > 0 && (
              <div className="mt-8 bg-gray-50 rounded-2xl p-6">
                ...
                <ul className="space-y-2">
                  {generatedContent.generated_content.usage_tips.map(
