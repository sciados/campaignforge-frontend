...
export default function CampaignDetailPage() {
  ...
  const loadCampaignData = useCallback(async () => {
    if (!campaignId || isInitializedRef.current || isLoadingRef.current) {
      console.log(
        "⏸️ Skipping loadCampaignData - already initialized/loading or no campaignId"
      );
      return;
    }
    ...
    try {
      ...
      try {
        const intelligence = await stableApi.getCampaignIntelligence(
          campaignId
        );
        if (intelligence && intelligence.intelligence_entries) {
          setIntelligenceData(intelligence.intelligence_entries);
          console.log(
            `📊 Loaded ${intelligence.intelligence_entries.length} intelligence sources`
          );
        } else {
          setIntelligenceData([]);
        }
  ...
  const refreshIntelligenceData = useCallback(async () => {
    ...
    try {
      ...
      if (intelligence && intelligence.intelligence_entries) {
        setIntelligenceData(intelligence.intelligence_entries);
        console.log(
          `✅ Refreshed: ${intelligence.intelligence_entries.length} intelligence sources`
        );
      }
    } catch (error) {
                          ...
                          {intelligenceData.length > 0
                            ? Math.round(
                                (intelligenceData.reduce(
                                  (acc: number, intel: any) =>
                                    acc + (intel.confidence_score || 0),
                                  0
                                ) /
                                  intelligenceData.length) *
                                  100
                      ...
                      <div>
                        <div className="text-2xl font-semibold text-gray-900">
                          {intelligenceData.reduce(
                            (acc: number, intel: any) =>
                              acc +
                              (intel.insights_extracted ||
                                Object.keys(intel.offer_intelligence || {})
                                  .length ||
                                1),
                            0
                          )}
                  ...
                  {intelligenceData.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <p className="text-gray-600 mb-6 max-w-md mx-auto">
                        This campaign needs to complete the analysis process
                        before content can be generated.
                      </p>
                      <button
                        onClick={() =>
